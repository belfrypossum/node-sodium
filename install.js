...
function recursePathList(paths) {
  ...
  if (!fs.existsSync(file)) {
    try {
      ...
      throw new Error(
        'Failed to create path: ' + file + ' with ' + e.toString()
      );
    }
  }
...
function createFullPath(fullPath) {
  ...
  if (0 === pathList.length) throw new Error('Path list was empty');
...
function download(url, dest, cb) {
  if (exists(dest)) {
    console.log(
      'File ' +
        dest +
        ' alredy exists, run make clean if you want to download it again.'
    );
    cb(null);
  }
...
function doDownloads(next) {
  var baseURL =
    'https://raw.githubusercontent.com/paixaop/libsodium-bin/master';
  ...
  switch (os.arch()) {
    ...
    default:
      throw new Error(
        'No pre-compiled binary available for this platform ' + os.arch()
      );
  }
...
function errorSetMSVSVersion() {
  console.log(
    'Please set your Microsoft Visual Studio version before you run npm install'
  );
...
function errorInvalidMSVSVersion() {
  console.log(
    'Invalid msvs_version ' + process.env.npm_config_msvs_version + '\n'
  );
  console.log(
    'Please set your Microsoft Visual Studio version before you run npm install'
  );
...
function checkMSVSVersion() {
  ...
  if (
    process.env.npm_config_msvs_version.search(
